...
type MergeStrategy = "" | "--no-ff" | "--ff-only";
...
interface MergeProfile extends MergeConfig {
  id?: string;
  label?: string;
  description?: string;
}
...
let lastConflictFiles: string[] = [];
let lastWorkspaceRoot: string | null = null;
let lastConfigRoot: string | null = null;
let lastConfigItems: ConfigSummaryItem[] = [];
let lastConfigError = "";
let lastUiLabels: UiLabels = DEFAULT_UI_LABELS;
...
class QuickMergeViewProvider implements vscode.WebviewViewProvider {
  ...
  private async checkoutOriginal(): Promise<void> {
    ...
    try {
      await runGit(
        ["checkout", lastFailureContext.originalBranch],
        lastFailureContext.cwd
      );
  ...
  private async postState(options?: { loadConfig?: boolean }): Promise<void> {
    ...
    try {
      ...
      if (loadConfig) {
        const { items, error, uiLabels } = await getConfigSummary(
          cwd,
          currentBranch,
          remotes
        );
  ...
  private setupGitWatchers(cwd: string): void {
  ...
  private disposeGitWatchers(): void {
...
async function getConfigSummary(
  ...
  currentBranch: string,
  remotes: string[]
): Promise<{ items: ConfigSummaryItem[]; error: string; uiLabels: UiLabels }> {
  try {
    const configFile = await readMergeConfig(cwd);
...
async function readMergeConfig(cwd: string): Promise<MergeConfigFile> {
  const configUri = vscode.Uri.file(path.join(cwd, CONFIG_FILE_NAME));
...
function normalizeConfigFile(configFile: MergeConfigFile): {
  uiLabels: UiLabels;
  profiles: MergeProfile[];
} {
...
function normalizeUiLabels(input?: UiLabels): UiLabels {
  return {
...
function getProfileKey(profile: MergeProfile, index: number): string {
...
function getProfileLabel(profile: MergeProfile, index: number): string {
...
function planLabel(label: string, plan: ResolvedMergePlan): string {
  if (label) {
    return label;
  }
...
function buildConfigSummary(plan: ResolvedMergePlan): string[] {
  const lines = [
...
function getDefaultConfigTemplate(): MergeConfigFile {
  return {
    ui: {
      refreshLabel: "刷新配置",
      openConfigLabel: "打开配置文件",
...
function getDefaultRemote(remotes: string[]): string | null {
...
function interpolateTemplate(input: string, context: Record<string, string>): string {
...
function getWorkspaceRoot(): string | null {
  ...
  if (lastWorkspaceRoot) {
    const matched = folders.find(
      (folder) => folder.uri.fsPath === lastWorkspaceRoot
    );
    if (matched) {
